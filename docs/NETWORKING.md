...
- **Token / identity** (planned) — signed tokens from an auth service, enabling
  persistent bans, stats and cross-server progression. A whole subsystem, so it is
  a hook rather than a requirement: the engine calls
  `onAuthenticate(peer, credentials)` and the game decides.

Host controls in all modes: kick, and ban by address. Those need no identity
system and stop the common case of one person ruining a session.
...
2. **`enet` transport**, listen mode, `direct` discovery. Two processes on one
   machine, then two machines on a LAN.
3. **`dedicated` mode** under headless LÖVE, plus `--server` flags.
4. **`lan` discovery** over UDP broadcast, and a server browser UI (needs the GUI
   toolkit from roadmap phase 3).
5. **`master` discovery**: announce/query protocol, reference registry, and the
   diagnostics above.
6. **Hole punching** through the master server.
7. **Steam transport** and SDR, behind the same interface.

Phases 1 to 3 make the thing work. Phase 4 makes it pleasant. Phases 5 to 7 make
