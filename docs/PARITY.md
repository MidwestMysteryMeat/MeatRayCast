...
| ~~Ambient sound zones~~ ✅ | Room tones | **C31 done** | `meatray.game.ambient` tracks which zone (smallest-wins on overlap) the listener is in and flags transitions; `ambient <sound> [storey] x1 y1 x2 y2` .map directive. Demo follows the player and would crossfade the owner's loop. |
...
| ~~Meta progression~~ ✅ | Between-campaign unlocks | **C23 done** | `meatray.game.progression`: currency/unlocks/stats over the storage backend; demo banks a campaign win. |
