...
function RelayT.new(opts)
    ...
    return setmetatable({
        ...
        session  = nil,
        secret   = nil,
        -- End-to-end data key (32 raw bytes). Generated by the host on open,
        -- carried in the ticket, never sent to the relay. nil means cleartext.
        dataKey  = opts.dataKey,
        encrypt  = opts.encrypt ~= false,  -- host generates a key unless false
        maxSlots = nil,
        ...
        stats = {
            ...
            sealed = 0, opened = 0, authFailed = 0,
        },
    }, RelayMT)
end
...
function RelayMT:listen(opts)
    ...
    -- End-to-end key: generated here, put in the ticket, never shown to the
    ...
    self:log(('session %s open on %s, up to %d players%s')
             :format(tostring(self.session), address, self.maxSlots or 0,
                     self.dataKey and ', end-to-end sealed' or ''))
...
function RelayMT:ticket()
    ...
    if self.dataKey then
        ticket.dataKey = Crypto.toHex(self.dataKey)
    end
    return Wire.formatTicket(ticket)
end

-- True when data frames on this session are sealed. Control frames to the
-- relay never are — the relay has to parse them.
function RelayMT:sealed()
...
function RelayMT:connect(address)
    ...
    -- The data key rides in the ticket, not on the join control line: the
    -- relay must never see it. A three-field ticket leaves dataKey nil and the
    -- session stays cleartext, matching the host that issued it.
    if ticket.dataKey then
        local key, keyErr = Crypto.fromHex(ticket.dataKey)
        if not key or #key ~= Crypto.KEY_BYTES then
            self.inner:close()
            self.link = nil
            return nil, keyErr or 'ticket data key is not 32 bytes'
        end
        self.dataKey = key
    end
...
-- Associated data binds the seal to the channel and reliability flag, so a
-- relay (or a man in the middle on either hop) cannot take a sealed snapshot
-- and re-emit it as a reliable join packet, or vice versa.
local function sealAad(channel, reliable)
    return string.char((channel or 0) % 256) .. ((reliable == false) and 'u' or 'r')
end

local function sealPayload(self, data, channel, reliable)
    if not self.dataKey then return data end
    local sealed, err = Crypto.seal(self.dataKey, data, sealAad(channel, reliable))
    if not sealed then
        self:log('seal failed: ' .. tostring(err))
        return nil
    end
    self.stats.sealed = self.stats.sealed + 1
    return sealed
end

local function openPayload(self, data, channel, reliable)
    if not self.dataKey then return data end
    -- Reliability is on the relaywire header byte, so the AAD matches what
    -- sealPayload bound at send time. A flipped reliability bit fails the tag
    -- rather than opening as the wrong kind of message.
    local plain = Crypto.open(self.dataKey, data, sealAad(channel, reliable ~= false))
    if plain then
        self.stats.opened = self.stats.opened + 1
        return plain
    end
    self.stats.authFailed = self.stats.authFailed + 1
...
function RelayMT:send(peer, data, channel, reliable)
    ...
    local payload = sealPayload(self, data, channel, reliable ~= false)
    if not payload then return false end

    local slot = (self.role == 'host') and peer.slot or 0
    local frame = Wire.data(slot, payload, reliable ~= false)
...
function RelayMT:broadcast(data, channel, reliable)
    ...
    self.idle = 0
    self.stats.sent = self.stats.sent + 1
    self.inner:send(self.link, Wire.broadcast(payload, reliable ~= false), channel, reliable)
end
...
function RelayMT:translate(event)
    ...
    local kind, a, b, reliable = Wire.parse(event.data)
    if not kind then
        self.stats.dropped = self.stats.dropped + 1
        return
    end
    ...
    local plain = openPayload(self, b, event.channel or 0, reliable)
    if not plain then return end

    self.stats.received = self.stats.received + 1
    push(self, { type = 'receive', peer = peer, data = plain, channel = event.channel })
end
